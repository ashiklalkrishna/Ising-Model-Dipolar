...
@njit
def flip(grid, coord, m, neighbor_list, t, N, J):  
    alpha = 1 
    for k in range(0,N):
        for l in range(0,N):
            ...
            for j in range(1,5):
                ...
                if neigh != -1:
                    ...
                    e_dip = alpha * np.dot(e_d, mi)
                    e_dip_flip = alpha * np.dot(e_d, mi_flip)

                    e_ex = -J * s * grid[neigh]
                    e_ex_flip = -J * s_flip * grid[neigh]

                    e += e_dip + e_ex
                    e_flip +=  e_dip_flip + e_ex_flip
...
for k in range(eqsteps):
    flip(grid, coord, m_vectors, neighbor_list, t, N, J)
